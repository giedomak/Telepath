...
package pathdb.kpathindex.benchmark;
...
import pathdb.kpathindex.pathindex.tree.SearchCursor;
...
public class BenchmarkTest {
  ...
  public static double performSearchExperiment(IndexTree tree, int items_to_insert, int number_of_paths)
      throws IOException {
    double totalSum = 0;
    long[] key = new long[4];
    for (int i = 1; i < items_to_insert; i++) {
      ...
      try {
        PageProxyCursor cursor = tree.disk.getCursor(result.pageID);
        if (!Arrays.equals(key, result.next(cursor))) {
  ...
  private static double performDeletionExperiment(IndexTree tree, int items_to_insert, int number_of_paths) {
    double totalSum = 0;
    long[] key = new long[4];
    for (int i = 1; i < items_to_insert; i++) {
      key[0] = (long) (i % number_of_paths);
  ...
  public static void logToFile(String text) {
    try (PrintWriter out = new PrintWriter(
        new BufferedWriter(new FileWriter("benchmarking_results.txt", true)))) {
      out.println(text);
