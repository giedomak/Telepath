...
/**
 * Split a [LogicalPlan] on Union into multiple ParseTrees.
 *
...
object LogicalPlanUnionPuller {
     ...
     *
     * @param logicalPlan The LogicalPlan to parse.
     * @return List of ParseTrees each not containing the UNION operator.
     */
    fun parse(logicalPlan: LogicalPlan): List<LogicalPlan> {

        val parseTrees = mutableListOf<LogicalPlan>()

        // TODO: Has to be refactored now that a logicalPlan doesn't have the isRoot property anymore.

//        parseTrees.add(logicalPlan)
//
//        // Print the parsed LogicalPlan
//        logicalPlan.print()
//
//        // List to hold the ParseTrees which UNIONS in them
//        var unionTrees = emptyList<LogicalPlan>()
//
//        // Collect the ParseTrees which contain the UNION operator
//        while ({
//            unionTrees = parseTrees.stream()
//                    .filter { it.contains(LogicalPlan.UNION) }
...
//                val clone = tree.clone() as LogicalPlan
//
...
//        }
     ...
     */
    private fun removeFirstUnion(tree: LogicalPlan, childChooserIndex: Int): Boolean {
        ...
        for (child in tree.children) {
            ...
            if (child.operator == LogicalPlan.UNION) {
